...
--21. Cho biết số lượng đề án do phòng 'Nghiên Cứu' chủ trì.
	SELECT COUNT(DEAN.MADA) as 'Số lượng đè án'
	FROM DEAN, PHONGBAN
...
SELECT DEAN.MADA, COUNT(DEAN.MADA) AS 'Số giờ làm việc'
FROM DEAN, CONGVIEC
...
SELECT DEAN.TENDEAN, COUNT(DEAN.MADA) AS 'Số giờ làm việc'
FROM DEAN, CONGVIEC
WHERE DEAN.MADA = CONGVIEC.MADA
GROUP BY DEAN.MADA, DEAN.TENDEAN
--27. Với mỗi đề án, cho biết có bao nhiêu nhân viên tham gia đề án đó.
	SELECT TENDEAN, COUNT(PHANCONG.MA_NVIEN) AS 'Số nhân viên tham gia'
	FROM DEAN, PHANCONG
	WHERE DEAN.TENDEAN = PHANCONG.MADA
